...
static void CG_DrawSaberStyle(centity_t *cent, menuDef_t *menuHUD, hudElement_t *parent)
{
	...
	if (!setupDone)
	{
		...
		focusItems[3] = Menu_FindItemByName(menuHUD, "saberstyle_dual");
		if (focusItems[3]) CG_HUD_CreateChildElement(parent, focusItems[3]->window.name, &focusItems[3]->window.rect.x, &focusItems[3]->window.rect.y, &focusItems[3]->window.rect.w, &focusItems[3]->window.rect.h, qfalse);
		focusItems[4] = Menu_FindItemByName(menuHUD, "saberstyle_staff");
